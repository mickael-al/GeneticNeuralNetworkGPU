...
void main()
{
	...
	if (nni.index[uint(double(nid) / double(nnd.sizePCurrent))] < nnm.numberOfBest)
	{
		return;
	}
	...
	for (uint i = 0; i < nnd.sizePPrevious; i++)//weight from current layer and use size of previous layer = number weight of current layer
	{
		...
		{
			nnv.data[off_inn + nnd.loc + off_link + i] += randomInRange(nid + hash(i * nnd.sizeNN) + random_uniform) * nnm.weightScale;
		}
	}
	uint activation_mutate = uint(double(nnm.activation) * 10000.0);
	if ((hash(nid) + random_uniform) % 10000 < activation_mutate)
	{
		nnv.data[off_inn + nnd.loc + off_link + nnd.sizePPrevious + 1] = float(hash(hash(nid) + random_uniform) % 9);
	}
}